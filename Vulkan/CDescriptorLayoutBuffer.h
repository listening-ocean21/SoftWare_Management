...
//总体功能
//传送全局变量（例如转换矩阵），比如发生在每一帧的数据，这是数据如果继续放在顶点数据（每个顶点都要加载一次，很耗带宽
//正确处理此问题的途径是使用资源描述符(resource descriptors)。描述符是着色器自由访问缓冲区和图像资源的一种方式。
//设置一个包含转换矩阵的缓冲区，并使顶点着色器通过描述符访问它们。
//描述符的使用由三部分组成：
//1.在管线创建时指定描述符的布局结构
//2.从描述符对象池中分配描述符集合
//3.在渲染阶段绑定描述符集合
//总之：描述符布局(descriptor layout)指定了管线访问的资源的类型，描述符集合(descriptor set)指定将绑定到描述符的实际缓冲区或映射资源，描述符集合会像顶点缓冲区和帧缓冲区一样被绑定到绘制命令
