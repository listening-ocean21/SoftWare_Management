...
//总体功能
//之前建立的顶点缓冲区使用内存类型并不是适合显卡读取的最佳内存类型
//最适合显卡读取的内存类型具有VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT标记，含有这一标记的内存类型通常CPU无法直接访问。
//这里，通过创建两个顶点缓冲区，一个缓冲区将CPU加载数据，另一个负责将CPU加载到缓冲中的数据复制到显卡可以快速读取的缓冲中
