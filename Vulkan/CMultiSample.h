...
private:
   ...
   //交换链图像由交换链自己负责创建，并在交换链清除时自动被清除，不需要我们自己进行创建和清除操作
	...
	void __pickPhysicalDevice()
	{
		...
		for (const auto& device : Devices)
		{
			if (__isDeviceSuitable(device))
			{
				...
				m_MsaaSamples = __getMaxUsableSampleCount();
				break;
			}
		}
	...
	void __createRenderPass()
	{
		...
		ColorAttachment.samples = m_MsaaSamples;  //做多重采样
		...
		ColorAttachment.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // 将finalLayout从VK_IMAGE_LAYOUT_PRESENT_SRC_KHR修改为VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL。这是因为多采样图像不能直接呈现。我们首先需要将它们解析为一个常规图像。这个要求不适用于深度缓冲区，因为它在任何时候都不会显示
		...
		//解析附件
		//将finalLayout从VK_IMAGE_LAYOUT_PRESENT_SRC_KHR修改为VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL。这是因为多采样图像不能直接呈现。我们首先需要将它们解析为一个常规图像。这个要求不适用于深度缓冲区，
	    //因为它在任何时候都不会显示。因此，只需要添加一个新的颜色附件，即所谓的解析附件:
		...
		//指示渲染通道将多采样的彩色图像解析为常规附件。创建一个新的附件引用，指向作为解析目标的颜色缓冲区:
	...
	void __createGraphicsPipelines()
	{
		...
		PipelineMultisampleStateCreateInfo.rasterizationSamples = m_MsaaSamples;
	...
	void __createTextureImage()
	{
		...
		__createImage(TexWidth, TexHeight, m_MipLevels, VK_SAMPLE_COUNT_1_BIT,VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
	...
	void __createDepthResources()
	{
		...
		__createImage(m_SwapChainExtent.width, m_SwapChainExtent.height, 1, m_MsaaSamples, DepthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_DepthImage, m_DepthImageMemory);
	...
	//获取最大采样量
	VkSampleCountFlagBits  __getMaxUsableSampleCount() {
		VkPhysicalDeviceProperties physicalDeviceProperties;
		vkGetPhysicalDeviceProperties(m_PhysicalDevice, &physicalDeviceProperties);

		//短板效果：同时考虑颜色和深度的采样数量—其中较小值将是我们能支持的最大值
	...
	void __createColorResources() {
		...
		__createImage(m_SwapChainExtent.width, m_SwapChainExtent.height, 1, m_MsaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_ColorImage, m_ColorImageMemory);
		m_ColorImageView  = __createImageView(m_ColorImage,1, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT);

		__transitionImageLayout(m_ColorImage,1, colorFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
	}
