...
//创建顶点缓冲区：缓冲区是内存中的一个区域，可以向显卡提供要读取的任意数据，缓冲1区不会自动分配内存空间
namespace test4
{
	const int WIDTH = 800;
		...
		private:
		   ...
		   //交换链图像由交换链自己负责创建，并在交换链清除时自动被清除，不需要我们自己进行创建和清除操作
			...
			void __creatSVertexBuffer()
			{
				...
				BufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; //指定缓冲区数据如何被使用
				BufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; //冲区也可以由特定的队列簇占有或者多个同时共享,在这里我们使用图形队列，使用独占访问模式
				//fferCreateInfo.flags = 0;  //配置稀疏内存缓冲区
				...
				if (vkAllocateMemory(m_Device, &MemoryAllocateInfo, nullptr, &m_VertexBufferMemory) != VK_SUCCESS)
				{
				...
				//填充顶点缓冲区,将顶点数据Copy到缓冲区,使用vkMapMemory将缓冲区内存映射到CPU可访问的内存中完成
				void* Data;
